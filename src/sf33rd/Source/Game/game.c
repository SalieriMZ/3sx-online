...
#ifdef __vita__
    {
        extern const FrameMetrics* SDLApp_GetFrameMetrics(void);
        const FrameMetrics* fm = SDLApp_GetFrameMetrics();
        if (fm != NULL) {
            // Rolling average over the metrics window so the readout doesn't
            // jitter every frame.
            float sum = 0.0f;
            const int n = (int)(sizeof(fm->fps) / sizeof(fm->fps[0]));
            for (int i = 0; i < n; i++) {
                sum += fm->fps[i];
            }
            const float avg_fps = sum / (float)n;
            char buf[16];
            SDL_snprintf(buf, sizeof(buf), "FPS %2.0f", avg_fps);
            SSPutStrPro(1, 4, 4, 9, 0xFFFFFFFF, buf);
        }
    }
#endif
}
...
// Set true by Netplay_Run's EXITING arm. Once a netplay match has run
// this session, the title-screen attract-mode demo loop is permanently
// disabled until the process restarts. Switching into the demo path
// while netplay-side state (gekko adapter, DPAPI cred handles, Discord
// RPC client) is still tearing down crashes both Windows (0xC0000005)
// and Mali GPU on Android. Cheap to suppress, expensive to debug all
// post-match state interactions with the original 1.6.9 attract path.
bool demo_disabled_after_netplay = false;

void Check_Back_Demo() {
#ifdef __vita__
    // attract-mode demo currently crashes in effect_M5_move on Vita hardware
    // (C2-12828-1). Disable entirely until the game-logic regression is traced.
    G_Timer = 0;
    return;
#endif
    if (demo_disabled_after_netplay) {
        G_Timer = 0;
        return;
    }
    if (++G_Timer < 1800) {
