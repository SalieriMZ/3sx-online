...
void ImGuiW_Init(SDL_Window* window, ImGui_ImplSDLGPU3_InitInfo* init_info) {
    ...
    SettingsPanel_Init();
    ImGuiW_RegisterPanel(SettingsPanel_Render);
    ImGuiW_RegisterPanel(SettingsPanel_RenderFpsOverlay);
...
#define IMGUIW_MAX_PANELS 8
static ImGuiW_PanelRenderFn panels[IMGUIW_MAX_PANELS];
static int panel_count = 0;

void ImGuiW_RegisterPanel(ImGuiW_PanelRenderFn fn) {
    if (panel_count >= IMGUIW_MAX_PANELS || fn == NULL) {
        return;
    }
    panels[panel_count++] = fn;
}

void ImGuiW_RenderPanels(void) {
    for (int i = 0; i < panel_count; i++) {
        panels[i]();
    }
}

void ImGuiW_DiscardFrame(void) {
    // Matches a previous ImGuiW_NewFrame() when the swapchain texture isn't
    // available (window minimized/resizing). Closes the frame so internal
    // widget state (especially text input) is finalized without producing draw
    // data. Without this, a subsequent NewFrame asserts.
    ImGui_EndFrame();
}
