...
void init_args(int argc, const char* argv[]) {
    struct argparse_option options[] = {
        ...
        OPT_BOOLEAN(0, "force-relay", &args.netplay.force_relay, "Force server relay (disable P2P).", NULL, 0, 0),
#endif
...
void Args_SetForceRelay(int v) {
    args.netplay.force_relay = v ? 1 : 0;
}
#endif
...
#if defined(__vita__)
    // Vita is WiFi-only with no in-game relay toggle — clear any stale flag.
    args.netplay.force_relay = 0;
    Config_SetBool(CFG_KEY_NETPLAY_FORCE_RELAY, false);
    return;
#endif

    if (args.netplay.force_relay == 0) {
        args.netplay.force_relay = Config_GetBool(CFG_KEY_NETPLAY_FORCE_RELAY) ? 1 : 0;
    }

#if defined(__ANDROID__)
    // Android default: force_relay=1 (mobile carriers often CGNAT).
    if (!Config_GetBool(CFG_KEY_NETPLAY_FORCE_RELAY)) {
        args.netplay.force_relay = 1;
        Config_SetBool(CFG_KEY_NETPLAY_FORCE_RELAY, true);
    }
#endif
}
#endif
