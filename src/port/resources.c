...
// libcdio isn't built for Android / Vita (ioctl-heavy CD-ROM stack with no
// equivalent on those platforms). On Android, MainActivity copies the AFS in
// Java-side via SAF before SDL starts; on Vita, the user is expected to
// preplace ux0:data/3sx/resources/SF33RD.AFS manually.
#if CRS_APP_DRIVER_SDL && !defined(__ANDROID__) && !defined(__vita__)
...
#ifdef __vita__
    // Bundled-AFS builds (vita/resources/SF33RD.AFS at build time) win;
    // otherwise read the user-staged copy under ux0:.
    static const char* APP0 = "app0:/resources/SF33RD.AFS";
    static const char* UX0  = "ux0:data/3sx/resources/SF33RD.AFS";
    return file_exists(APP0) ? APP0 : UX0;
#else
    if (afs_path == NULL) {
...
bool Resources_Check() {
    ...
    SDL_Log("Resources_Check: path=%s exists=%d", afs_path, afs_present ? 1 : 0);
...
// Vita: SF33RD.AFS must be placed at ux0:data/3sx/resources/SF33RD.AFS by the
// user before launch (no SAF-equivalent, no native file picker for ISOs).
bool Resources_RunResourceCopyingFlow() {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,
                             "Missing ROM",
                             "Place SF33RD.AFS into ux0:data/3sx/resources/ and relaunch.",
                             NULL);
    return false;
}
...
// MainActivity.onCreate copies the AFS into <prefpath>/resources/SF33RD.AFS
// via SAF before SDL is initialised. Resources_Check is the gate for the
// COPYING_RESOURCES phase - if it returns false here, the game can't help
// (no libcdio, no native file picker). Show a fatal message and bail.
bool Resources_RunResourceCopyingFlow() {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,
                             "Missing ROM",
                             "Place SF33RD.AFS into the app data directory and relaunch.",
                             NULL);
    return false;
}

#elif CRS_APP_DRIVER_SDL
