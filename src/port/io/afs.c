...
// Weak hook fired between chunks so the netplay layer can keep the TCP
// channel + LOADING signal alive during slow flash reads.
void AFS_OnSyncTick(void) __attribute__((weak));
...
void AFS_ReadSync(AFSHandle handle, void* buf) {
    ...
    while (request->state == AFS_READ_STATE_READING) {
        const size_t before = request->bytes_read;
        read_into_request(request, _read_chunk_size);
        if (request->bytes_read == before) {
            request->state = AFS_READ_STATE_ERROR;
            break;
        }
        AFS_OnSyncTick();
    ...
    SDL_assert(request->state == AFS_READ_STATE_FINISHED || request->state == AFS_READ_STATE_ERROR);
}
