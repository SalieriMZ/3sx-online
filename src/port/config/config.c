...
static const ConfigEntry default_entries[] = {
    ...
    { .key = CFG_KEY_SHOW_HITBOXES, .type = CFG_BOOL, .value.b = false },
    { .key = CFG_KEY_SHOW_FPS, .type = CFG_BOOL, .value.b = false },
    { .key = CFG_KEY_HUD_SCALE_PCT, .type = CFG_INT, .value.i = 100 },
    { .key = CFG_KEY_DISCORD_RPC, .type = CFG_BOOL, .value.b = true },
};
...
static void write_defaults(const char* dst_path) {
    ...
    io_printf(io,
              "# For the full list of settings see https://github.com/SalieriMZ/3sx-online/blob/main/docs/config.md\n\n");
...
static void persist_all_entries(void) {
    const char* pref_path = Paths_GetPrefPath();
    char* config_path;
    char* tmp_path;
    SDL_asprintf(&config_path, "%sconfig", pref_path);
    SDL_asprintf(&tmp_path, "%sconfig.tmp", pref_path);

    SDL_IOStream* io = SDL_IOFromFile(tmp_path, "w");
    if (io == NULL) {
        SDL_Log("Config: failed to open %s for write: %s", tmp_path, SDL_GetError());
        SDL_free(config_path);
        SDL_free(tmp_path);
        return;
    }

    // Write merged: defaults overridden by entries[]
    for (int i = 0; i < (int)SDL_arraysize(default_entries); i++) {
        const ConfigEntry* def = &default_entries[i];
        const ConfigEntry* override_entry = find_entry_in_array(def->key, entries, entry_count);
        print_config_entry_to_io(io, override_entry ? override_entry : def);
        io_printf(io, "\n");
    }
    SDL_CloseIO(io);

#ifdef _WIN32
    // rename() does not overwrite an existing file on Windows. Remove the
    // target first; loses POSIX atomicity but works.
    remove(config_path);
#endif
    if (rename(tmp_path, config_path) != 0) {
        SDL_Log("Config: rename %s -> %s failed", tmp_path, config_path);
    }
...
static ConfigEntry* upsert_entry(const char* key) {
    ConfigEntry* existing = find_entry_in_array(key, entries, entry_count);
    if (existing != NULL) {
        if (existing->type == CFG_STRING && existing->value.s != NULL) {
