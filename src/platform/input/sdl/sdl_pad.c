...
static void handle_gamepad_added_event(SDL_GamepadDeviceEvent* event) {
    ...
    // Keyboard owns slot 0 by convention so a single-player keyboard user is
    // always P1, even when an idle gamepad is plugged in. New gamepads go to
    // the next free slot (1, 2, ...).
    for (int i = 0; i < INPUT_SOURCES_MAX; i++) {
        SDLPad_InputSource* input_source = &input_sources[i];
...
static void get_keyboard_state(Input_ButtonState* state) {
    ...
    if (io != NULL && io->WantCaptureKeyboard) {
        // Game input suppressed while ImGui has focus (settings panel, chat).
        return;
    }
#endif
