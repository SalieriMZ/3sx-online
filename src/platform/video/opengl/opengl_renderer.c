...
#ifdef __vita__
// vitaGL provides GL ES 2.0 entry points directly; no loader needed.
#include <vitaGL.h>
// GLES2 lacks GL_R8UI / GL_RED_INTEGER (a GLES3/GL3.3 feature). Palette
// textures will render wrong on Vita until proper GLES3 path is wired, but
// the tokens below let the code link. TODO: rewrite palette path with
// GL_LUMINANCE or sampler arrays for visual correctness.
#ifndef GL_R8UI
#define GL_R8UI GL_LUMINANCE
#endif
#ifndef GL_RED_INTEGER
#define GL_RED_INTEGER GL_LUMINANCE
#endif
#else
#include "glad.h"
#endif
...
#ifdef __vita__
    // vitaGL has no GL_TEXTURE_1D support (UNPURE_TEXFORMATS=0 in our build).
    // Use a Nx1 2D texture instead; the Vita-specific fragment shaders sample
    // it via texture2D(uPalette, vec2(u, 0.5)).
    glBindTexture(GL_TEXTURE_2D, palette);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, internal_format, color_count, 1, 0, format, type, pixels);
#else
...
static SDL_Window* OpenGLRenderer_Init(const SDLRenderBackendInitInfo* init_info) {
...
#ifdef __vita__
    // vitaGL owns the framebuffer; SDL3-Vita has no GL backend so don't pass
    // SDL_WINDOW_OPENGL and don't call SDL_GL_*. vglInit must run before any
    // GL call; legacy_pool_size = 0x800000 (8 MB) is the upstream default.
    ...
    gl_context = (SDL_GLContext)1; // sentinel; unused on Vita
#else
...
#ifdef __vita__
    // LINEAR upscale on the final canvas → screen blit smooths text + UI
    // at non-integer scales. Paired with scale_mode = NEAREST in sdl_app so
    // the 4:3 letterbox area gets filled (~725x544), not just the 2x integer
    // multiple (768x448 inside a much bigger panel).
...
#ifdef __vita__
    // vitaGL rejects GL_DEPTH_COMPONENT as a texture internal format and
    // GL_DEPTH_ATTACHMENT for glFramebufferTexture2D. SF3 is 2D so depth isn't
    // strictly needed; skip the depth attachment on Vita to keep the FBO valid.
    canvas_depth_tex = 0;
...
static void OpenGLRenderer_RenderFrame(SDL_Rect viewport) {
    ...
    // Hold last frame on stalls: skip canvas redraw so the FBO retains its
    // previous contents; the blit-to-window section still runs.
    extern bool game_drew_this_frame;
    if (!game_drew_this_frame) {
        goto blit_canvas;
    }
...
#ifdef __vita__
                glBindTexture(GL_TEXTURE_2D, quad->texture_spec.palette);
#else
...
blit_canvas:;
    ...
    glBindVertexArray(0);
#ifdef __vita__
    // Pass GL_TRUE so vitaGL composites any active Sce system dialog over the framebuffer.
    extern bool vita_common_dialog_active;
    vglSwapBuffers(vita_common_dialog_active ? GL_TRUE : GL_FALSE);
#else
