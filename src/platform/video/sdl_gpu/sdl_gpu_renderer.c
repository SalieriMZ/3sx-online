...
// Snapshot of the most recent non-empty quads array. Re-bound to `quads` on a
// frame where nothing was drawn (e.g., GekkoNet stalled this render frame or
// every advance event was rolling_back) so the renderer never clears to black.
static _Quad* quads_prev = NULL;
...
static SDL_GPUShader* create_shader(
    ...
    // Paths_GetBasePath returns SDL_GetBasePath on desktop but is overridden
    // on Android (where SDL_GetBasePath is NULL) to point at Paths_GetPrefPath
    // so shaders that were staged into <filesDir> resolve.
...
static SDL_Window* SDLGPURenderer_Init(const SDLRenderBackendInitInfo* init_info) {
    ...
    SDL_Log("3sx: SDL_CreateWindow flags=0x%llx %dx%d",
            (unsigned long long)init_info->window_flags,
            init_info->window_width, init_info->window_height);
    ...
    SDL_Log("3sx: window OK ptr=%p", (void*)window);

    SDL_Log("3sx: SDL_CreateGPUDeviceWithProperties (SPIRV|MSL, no clip distance)");
    // Mali-G615 (Redmi Lapis) reports shaderClipDistance=0. Default
    // SDL_CreateGPUDevice asks Vulkan device for clip distance support and
    // rejects any device that lacks it → "Failed to determine a suitable
    // physical device" on Mali. 3SX shaders don't use gl_ClipDistance, so
    // opt the feature out via the SDL3 3.4 property API. Same property API
    // also lets us skip depth clamping + indirect-draw-first-instance if
    // future devices fail those too — left at default (true) for now.
    ...
    // SDL3 3.4.8 added a stricter debug-mode assertion that rejects integer
    // texture formats (R8_UINT etc.) when their usage includes SAMPLER. 3SX's
    // palette pipeline writes 8-bit indices to R8_UINT textures and the
    // fragment shader reads them via .Load() — which is `usage = SAMPLER` on
    // SDL's side, even though no actual filtering happens. The Mali driver
    // copes fine. Turning debug_mode off skips the whole validation block
    // (and the few hundred microseconds it cost per Create call). If we ever
    // hit a real driver bug we can flip this back on temporarily.
    ...
    SDL_Log("3sx: device OK driver=%s", SDL_GetGPUDeviceDriver(device));

    SDL_Log("3sx: SDL_ClaimWindowForGPUDevice");
    SDL_ClaimWindowForGPUDevice(device, window);
    SDL_Log("3sx: claim OK");

#ifdef __ANDROID__
    // Android frame pacing is delegated to the swapchain present block — the
    // desktop sleep-loop overshoots by ~15ms on Mali drivers and collapses to
    // ~20 FPS. FIFO is guaranteed-supported on Vulkan and actually blocks at
    // vsync (~16.66ms @ 60Hz). MAILBOX is non-blocking on Mali — frames render
    // as fast as the GPU can chew them, which on Mali-G615 was 500+ FPS and
    // made the fixed-tick game logic run 8× too fast.
    present_mode = SDL_GPU_PRESENTMODE_VSYNC;
    SDL_Log("Using VSYNC present mode (Android: caps at display refresh)");
#else
...
static void SDLGPURenderer_RenderFrame(SDL_Rect viewport) {
    ...
    // Hold last frame on stalls: substitute previous-frame quads so overlay-only
    // frames don't render as black flicker.
    extern bool game_drew_this_frame;
    if (!game_drew_this_frame && arrlen(quads_prev) > 0) {
        arrsetlen(quads, arrlen(quads_prev));
        SDL_memcpy(quads, quads_prev, arrlen(quads_prev) * sizeof(_Quad));
    }
    ...
    // Snapshot quads before clearing so a subsequent empty frame can reuse it.
    if (arrlen(quads) > 0) {
        arrsetlen(quads_prev, arrlen(quads));
        SDL_memcpy(quads_prev, quads, arrlen(quads) * sizeof(_Quad));
    }
    arrsetlen(quads, 0);
