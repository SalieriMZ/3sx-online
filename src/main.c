...
#if NETPLAY_ENABLED
    {
        const NetplaySessionState nps = Netplay_GetSessionState();
        ...
        // During an active netplay session step_game owns sim + 2D draw. Calling
        // KnjFlush here would flip the kanji text double-buffer with no text
        // staged this frame, leaving step_game's text in the wrong slot --
        // visible as text flicker on character select / HUD.
        flFlip(0);
    }
#else
    njUserMain();
